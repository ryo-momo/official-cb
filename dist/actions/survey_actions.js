...
function handleBasicInfoUpdateOrReference(user) {
    user.response.message = [
        {
            type: 'text',
...
function handleSearchConditionUpdateOrReference(user) {
    user.response.message = [
        {
            type: 'text',
            text: '行いたい操作を選択してください。',
...
function endSurveyAction(user, current_survey_id) {
...
function setQuestion(user, current_question) {
    // Initialize message explicitly
    let message;
    if (current_question.design) {
        // Flex message
