...
const validateTextType = (answer_text) => {
...
const validateChoiceType = (answer_text, options) => {
...
const validateNumberType = (answer_text) => {
...
const surveyValidator = (user, answer_text) => {
    ...
    switch (current_question.id) {
        ...
        case 'yield':
            if ('options' in current_question) {
                validation_result = validateChoiceType(answer_text, current_question.options.filter((option) => { var _a; return !((_a = user.current_answers) === null || _a === void 0 ? void 0 : _a.includes(option.text)); }));
            }
            else {
                validation_result = { isValid: false, error_message: ERROR_MESSAGES.NO_OPTIONS };
            }
